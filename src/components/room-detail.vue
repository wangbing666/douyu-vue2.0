  ...
  <div class="video-view">
    ...
    <div class="player">
        ...
        <div class="poster" v-show="isPlay">
            <img :src="room.room_thumb" id="video-poster">
            <div class="play-btn" @click="playVideo"><i class="icon icon-play2"></i></div>
            <div class="room-info">
                <span class="name">{{room.room_name}}</span>
                <span class="class">{{room.cate_name}}</span>
            </div>
        </div>
    </div>  
    <div class="info-bar">
      <div class="live-info">
        <div class="nickname">主播: <span>{{room.owner_name}}</span></div>
    ...
    <div class="favors">
      ...
      <div class="live-list clearfix">
        <hot-live v-for="hot in hots" :hot="hot"></hot-live>
      </div>
      <div class="more-button" v-if="limit < 30" @click="getHotLive">
           更多热门直播
      </div>
    </div>
  </div>
...
export default{
    ...
    methods: {
      ...
      getHotLive () {
        ...
        if (this.limit > 30) { return false }
        this.$http.get('/api/live?limit='+this.limit)
        .then(response => {
          let data = response.data
          let json = data.data
          if (data.error === 0) {
